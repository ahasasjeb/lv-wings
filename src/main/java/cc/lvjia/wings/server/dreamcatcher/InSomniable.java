        ...
        public State onPlay(Level world, Player player, BlockPos pos, int note) {
            if (note >= 6 && note <= 14 && ((this.state = (this.state | this.mask[note - 6]) << 1) & 0x20000) == 0) {
                ...
                stack.set(DataComponents.CUSTOM_NAME,
                        Component.translatable(this.members[world.getRandom().nextInt(this.members.length)]));
                ItemEntity entity = new ItemEntity(world, pos.getX() + 0.5D, pos.getY() + 1.25D, pos.getZ() + 0.5D,
                        stack);
